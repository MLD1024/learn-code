package com.example.rpc.ioc.core;

import com.example.rpc.core.RpcClient;
import com.example.rpc.ioc.annotation.Autowired;
import com.example.rpc.ioc.annotation.Component;
import com.example.rpc.ioc.annotation.Qualifier;
...
public class ApplicationContext {
    ...
    private void doInjectDependencies(Object bean) {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (field.isAnnotationPresent(Reference.class)) {
                ...
                Object dependency = getDubboBean(field.getType());
                if (Objects.isNull(dependency) && reference.required()) {
                    throw new RuntimeException("Unsatisfied dependency: " + field.getType().getName());
                }
                try {
                    field.setAccessible(true);
    ...
    public Object getDubboBean(Class<?> requiredType) {
        try {
            RpcClient client = new RpcClient("localhost", 8080);
            client.connect();
            return RpcProxyFactory.createProxy(requiredType, client);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
